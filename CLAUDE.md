...
- [ ] `.github/PULL_REQUEST_TEMPLATE.md` のテンプレートを読んだ
- [ ] ベースブランチが正しいか確認した（通常は `develop`）
- [ ] テンプレートの全セクションを埋めた
...
`.github/PULL_REQUEST_TEMPLATE.md` に従い、以下を必ず記載:
...
# 2. PRを作成（developブランチへ）
gh pr create --base develop --title "機能: XXXを追加" --body "$(cat <<'EOF'
## 概要
XXX機能を追加
